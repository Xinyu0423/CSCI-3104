...
def findPath(graph,dest):
    if dest!="Spider":
        count = 0
        myList = list(graph.predecessors(dest))
        # this function return all parents node from destination
        # get this from TA
        for i in myList:
            count = count + findPath(graph, i)
        return count
    elif dest=="Spider":
        return 1
...
def main():
    #Question3()
    myGraph=Question5()
    count=findPath(myGraph,"Fly")
    print("the path from fly to spider is ", count)
